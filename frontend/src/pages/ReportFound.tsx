...
export const ReportFound: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await itemService.createFoundItem(formData);
    ...
    return (
      <div className="max-w-4xl mx-auto py-8 px-4 space-y-8 animate-in fade-in duration-300">
        <div className="glass-card rounded-2xl p-8 border border-emerald-500/30 text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-white">Found Item Report Logged Successfully!</h2>
        ...
        <div className="space-y-4">
          ...
          {submissionResult.matches.length === 0 ? (
            <div className="glass-card rounded-2xl p-8 text-center text-slate-400 text-xs">
              No immediate high-confidence matches were found against current reported lost items. The system will continue scanning future submissions.
            </div>
          ) : (
      ...
      <div className="glass-card rounded-2xl p-8 border border-emerald-500/20 space-y-6">
        <div className="space-y-2">
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            PUBLIC FOUND ITEM REPORT
          </span>
          <h1 className="text-2xl font-extrabold text-white">Report a Found Item</h1>
          <p className="text-xs text-slate-400">
            No registration required. Log a found item to instantly check for matching lost items.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="bob@example.com or +1-555-0843"
                    className="w-full glass-input rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder-slate-500"
                  />
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300">Date Found *</label>
              <input
                type="date"
                required
                value={eventDate}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300">Approximate Time Found</label>
              <input
                type="time"
                value={eventTime}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-300">Location Found *</label>
            <input
              type="text"
              required
              value={location}
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Describe condition, exact spot found, clear protective cover details..."
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-3.5 rounded-xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 text-white hover:from-emerald-500 hover:to-teal-500 transition-all flex items-center justify-center space-x-2 shadow-lg shadow-emerald-600/20"
          >
            <span>{submitting ? 'Submitting & Matching...' : 'Submit Found Item Report'}</span>
